...
class MessageWatcher {
    ...
    /**
     *  Извлекаем сообщения, которые были отправлены в основной мессенджер, но не получили подтверждение отправки,
     *  если в настройке указан допю мессенджер отправляем через него
     */
    ...
    async _send(sender, messages) {
        ...
        for (let message of messages) {
            let result
            if (message.qrcode && sendHelper.sendFile) {
                const qr = await makeQR(message.message)
                if (!qr) return
                result = await sendHelper.sendFile(message.peer, qr, message.message)
            } else {
                result = await sendHelper.send(message.peer, message.message)
            }
